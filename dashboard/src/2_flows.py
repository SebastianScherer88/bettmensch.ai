...
def display_flow_summary_table(formatted_flow_data: Dict) -> pd.DataFrame:
    ...
    flow_summary_df = pd.DataFrame(records)[['name','uid','creation_timestamp','progress','phase']]. \
        rename(columns={'name':'Name','uid':'ID','creation_timestamp':'Created','progress':'Progress','phase':'Phase'}). \
...
def get_formatted_flow_data(submitted_flows, flow_names) -> Dict:
    ...
    for i, (resource_name, submitted_flow) in enumerate(zip(flow_names,submitted_flows)):
        try:
            flow_dict = Flow.from_argo_workflow_cr(submitted_flow).model_dump()
            formatted_flow_data['object'][resource_name] = Flow.from_argo_workflow_cr(submitted_flow)
            ...
            print(e)
            st.write(f"Oops! Could not collect data for Flow {resource_name}: {e} Please make sure the Argo Workflow was created with the bettmensch.ai SDK and was submitted successfully.")
...
def display_flow_dag(formatted_flow_data, selected_flow, display_flow_ios) -> Tuple[DagVisualizationSchema,Dict]:
    """_summary_
    ...
    dag_visualization_schema = formatted_flow_data['object'][selected_flow].create_dag_visualization_schema(display_flow_ios)
...
def display_flow_dag_selection(formatted_flow_data: Dict, selected_flow: str, dag_visualization_element, tab_container_height: int=600):
    ...
    try:
        ...
        if element_is_task_node:
            ...
            task_inputs_tab, task_outputs_tab, task_script_tab, task_state_tab = st.tabs(["Task Inputs","Task Outputs","Task Script", "Task State"])
            ...
            with st.container(height=tab_container_height, border=False):
                with task_state_tab:
                    task_state = dict([(k,v) for k,v in task.items() if k in ('id','pod_name','host_node_name','phase','logs')])
                    task_state_formatted_df = pd.DataFrame(task_state,index=[0]). \
                        rename(columns={
                                'id':'Flow Task ID',
                                'pod_name': 'Pod',
            ...
            st.write(f"Select a task by clicking on the corresponding {PIPELINE_NODE_EMOJI_MAP['task']} node.")
        
    except TypeError as e:
        st.markdown(f"### {PIPELINE_NODE_EMOJI_MAP['task']} Task: None")
        st.write(f"Select a task by clicking on the corresponding {PIPELINE_NODE_EMOJI_MAP['task']} node.")
                    
def display_selected_flow(formatted_flow_data,selected_flow, chart_container_height: int = 700, col_container_height: int = 750, tab_container_height:int = 600):
    ...
    with flow_col:
        with st.container(height=col_container_height):
            ...
            tab_inputs, tab_metadata, tab_dag, tab_templates = st.tabs(["Flow Inputs", "Flow Meta Data", "Flow DAG", "Flow Templates",])
                
            with tab_inputs:
                with st.container(height=tab_container_height,border=False):
                    flow_inputs = formatted_flow_data['inputs'][selected_flow]
                    flow_inputs_formatted_df = pd.DataFrame(flow_inputs). \
                        rename(columns={
                                'name':'Name',
                                'value': 'Default',
                            }
            ...
            with tab_metadata:
                with st.container(height=tab_container_height,border=False):
                    st.markdown("### Spec")
                    st.json(formatted_flow_data['metadata'][selected_flow],expanded=True)
            ...
            with tab_templates:
                with st.container(height=tab_container_height,border=False):
                    st.markdown("### Spec")
                    st.json(formatted_flow_data['templates'][selected_flow],expanded=True)
        
    # display task level data
    with task_col:
        with st.container(height=col_container_height):
            display_flow_dag_selection(formatted_flow_data, selected_flow, dag_visualization_element, tab_container_height)
...
def main():
    ...
    dag_visualization_schema, dag_visualization_element = display_selected_flow(formatted_flow_data,selected_flow)
