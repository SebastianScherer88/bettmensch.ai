...
def hex_to_channel(hex: str, channel:Literal['RGB','BGR']):
    if channel == 'RGB':
        ...
        raise ValueError(f'Channel argument {channel} is invalid. Supported channels: RGB,BGR')
...
def color_background(image, new_background_color: Optional[Tuple[int,int,int]]=hex_to_bgr(DarkTheme().backgroundColor)):
...
def get_workflow_templates(configuration) -> List[IoArgoprojWorkflowV1alpha1WorkflowTemplate]:
