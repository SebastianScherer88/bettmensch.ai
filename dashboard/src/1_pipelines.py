...
def display_pipeline_dag(formatted_pipeline_data, selected_pipeline, display_pipeline_ios) -> Tuple[DagVisualizationSchema,Dict]:
    """_summary_
    ...
    Args:
        ...
        display_pipeline_ios (bool): The toggle value of the user selected pipeline dag I/O detail level in the flow chart.
...
def display_pipeline_dag_selection(formatted_pipeline_data: Dict, selected_pipeline: str, dag_visualization_element, tab_container_height: int=600):
    """Generates a tabbed, in depth view of the user selected DAG task node element.

    Args:
        formatted_pipeline_data (Dict): The formatted pipeline data as generated by `get_formatted_pipeline_data`
    ...
    try:
        ...
        if element_is_task_node:
            ...
            task_inputs_tab, task_outputs_tab, task_script_tab = st.tabs(["Task Inputs","Task Outputs","Task Script"])
...
def display_selected_pipeline(formatted_pipeline_data,selected_pipeline, chart_container_height: int = 700, col_container_height: int = 750, tab_container_height:int = 600):
    """Utility to display DAG flow chart and all relevant specs in tabbed layout for a user selected pipeline.

    Args:
        formatted_pipeline_data (Dict): The formatted pipeline data as obtained by `get_formatted_pipeline_data`.
        selected_pipeline (str): The name of the user selected pipeline.
    """
    ...
    with pipeline_col:
        with st.container(height=col_container_height):
            st.markdown(f"### :twisted_rightwards_arrows: Pipeline: `{selected_pipeline}`")
            
            tab_inputs, tab_metadata, tab_dag, tab_templates = st.tabs(["Pipeline Inputs", "Pipeline Meta Data", "Pipeline DAG", "Pipeline Templates",])
    ...
    with task_col:
        with st.container(height=col_container_height):
            display_pipeline_dag_selection(formatted_pipeline_data, selected_pipeline, dag_visualization_element, tab_container_height)
