...
def get_formatted_pipeline_data(registered_pipelines, pipeline_names) -> Dict:
    ...
    for i, (resource_name, registered_pipeline) in enumerate(
        ...
        try:
            ...
            print(e)
            st.write(
                f"Oops! Could not collect data for Pipeline {resource_name}: {e} Please make sure the workflow template was created with the bettmensch.ai SDK and was submitted successfully."
            )
...
def display_selected_pipeline(
    ...
    with pipeline_col:
        with st.container(height=col_container_height):
            st.markdown(
                f"### :twisted_rightwards_arrows: Pipeline: `{selected_pipeline}`"
            )
