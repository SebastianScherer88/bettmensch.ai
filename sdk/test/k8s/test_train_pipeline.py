...
def test_train_transformer_pipeline_3n_1p_decorator_and_register_and_run(
    test_output_dir, test_namespace
):
    train_transformer_pipeline_3n_1p.export(test_output_dir)

    assert not train_transformer_pipeline_3n_1p.registered
    assert train_transformer_pipeline_3n_1p.registered_id is None
    assert train_transformer_pipeline_3n_1p.registered_name is None
    assert train_transformer_pipeline_3n_1p.registered_namespace is None

    train_transformer_pipeline_3n_1p.register()

    assert train_transformer_pipeline_3n_1p.registered
    assert train_transformer_pipeline_3n_1p.registered_id is not None
    assert train_transformer_pipeline_3n_1p.registered_name.startswith(
        f"pipeline-{train_transformer_pipeline_3n_1p.name}-"
    )
    assert (
        train_transformer_pipeline_3n_1p.registered_namespace == test_namespace
    )  # noqa: E501

    train_transformer_flow = train_transformer_pipeline_3n_1p.run(
        inputs={
            "dataset": "multi30k",
            "source_language": "de",
            "target_language": "en",
