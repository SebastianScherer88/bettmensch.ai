...
def test_outputs_to_hera(
    ...
    """Test the OutputArtifact's and the OutputParameter's `to_hera` method for all currently supported scenarios."""
    test_owner = request.getfixturevalue(test_owner_fixture_name)
    test_io.set_owner(test_owner)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            Artifact(
                name="test_name",
                from_="{{tasks.mock-component-0.outputs.artifacts.test_source_name}}",
            ),
            False,
        ),
    ],
...
def test_input_artifact_to_hera(
    ...
    """Test the InputArtifact's `to_hera` method for all currently supported scenarios."""
    ...
    assert test_io.to_hera(template=is_template) == test_expected_hera_parameter


@pytest.mark.parametrize(
    "test_owner_fixture_name,test_source,test_source_owner_fixture_name,test_expected_hera_parameter",
    [
        (
            "test_mock_pipeline",
            None,
        ...
        (
            ...
            Parameter(
                name="test_name",
                value="{{tasks.mock-component-0.outputs.parameters.test_source_name}}",
            ),
        ),
        (
            "test_mock_component",
...
def test_input_parameter_to_hera(
    ...
    """Test the InputParameter's `to_hera` method for all currently supported scenarios."""
