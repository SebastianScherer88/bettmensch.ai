...
class ComponentInlineScriptRunner(InlineScriptConstructor):
    ...
    def _get_script_preprocessing(self, instance: Script) -> str:
        ...
        actual_input_parameters = [input_param for input_param in inputs.parameters if input_param.name not in output_names]
        ...
        preprocess = textwrap.dedent(preprocess) if preprocess != "\n# --- preprocessing\nimport json\n" else ""
...
class Component(object):
    ...
    def __init__(self,func: Callable, hera_template_kwargs: Dict = {}, **component_inputs_kwargs: Union[PipelineInput,ComponentOutput]):
    ...
    def generate_inputs_from_func(self,func: Callable, inputs: Dict[str,Union[PipelineInput,ComponentOutput]]) -> Dict[str,ComponentInput]:
        ...
        for name, input in inputs.items():
            ...
            if name not in func_inputs:
                raise ValueError(f"Attempting to declare unknown component input {name}. Known inputs: {func_inputs}.")
            ...
            if isinstance(input,PipelineInput):
                ...
                raise TypeError(f"Input {input} must be of type PipelineInput or ComponentOutput.")
    ...
    def build_hera_task_factory(self) -> Callable:
        ...
        script_decorator_kwargs.update(
            {
                'outputs':[
                    Parameter(name=output.name,value_from=models.ValueFrom(path=output.path)) for output in self.outputs.values()
                ]
            }
        )
...
def component(func: Callable) -> Callable:
    ...
    def component_factory(hera_template_kwargs: Dict =  {}, **component_inputs_kwargs) -> Component:
        
        return Component(func=func,hera_template_kwargs=hera_template_kwargs,**component_inputs_kwargs)
